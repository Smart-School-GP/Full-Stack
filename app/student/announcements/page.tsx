    ...
    <div className="page-container">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Stay updated with the latest school news and curriculum updates.
          </p>
        </div>
        ...
        <div className="flex p-1 bg-slate-100 dark:bg-slate-800/50 rounded-xl self-start">
          ...
          >
            ...
            Curriculum
            {curriculumCount > 0 && (
              <span className="ml-1 px-1.5 py-0.5 text-[10px] bg-brand-100 text-brand-600 rounded-full">
                {curriculumCount}
              </span>
            )}
          </button>
          ...
          >
            ...
            General
            {generalCount > 0 && (
              <span className="ml-1 px-1.5 py-0.5 text-[10px] bg-slate-200 text-slate-600 rounded-full">
                {generalCount}
              </span>
            )}
          </button>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(i => (
            <div key={i} className="card animate-pulse h-48 bg-slate-100 dark:bg-slate-800/50" />
          ))}
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-800/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700">
          ...
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white">No announcements found</h3>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Check back later for new updates.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {filteredAnnouncements.map((announcement) => (
            <div
              ...
              className={`group relative card overflow-hidden transition-all hover:scale-[1.02] cursor-pointer ${
                !announcement.isRead ? 'ring-2 ring-brand-500/20 bg-brand-50/30 dark:bg-brand-500/5' : ''
            ...
            >
              ...
              <div className="absolute top-4 right-4 flex items-center gap-2">
                ...
                {!announcement.isRead && (
                  <span className="flex h-2.5 w-2.5 rounded-full bg-brand-500 ring-4 ring-brand-500/20" />
                )}
              </div>
              ...
              <div className="p-6">
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 group-hover:text-brand-600 transition-colors line-clamp-1">
                  {announcement.title}
                </h3>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-700/50">
                  ...
                  <div className="text-brand-600 dark:text-brand-400 opacity-0 group-hover:opacity-100 transition-all flex items-center gap-1 text-xs font-bold">
                    Read More
                    <ChevronRightIcon />
                  </div>
                </div>
      ...
      {selectedAnnouncement && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          >
            {/* Modal Header */}
            <div className="relative h-32 bg-gradient-to-r from-brand-600 to-brand-800 p-8">
              <button
            ...
            <div className="p-8 overflow-y-auto max-h-[calc(90vh-8rem)]">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {selectedAnnouncement.creator?.role === 'admin' ? 'School Administrator' : 'Subject Teacher'}
                    </p>
                  </div>
                </div>
              ...
              {selectedAnnouncement.subject && (
                <div className="mt-8 p-4 bg-slate-50 dark:bg-slate-700/30 rounded-2xl flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Related Subject</p>
                    <p className="text-sm font-bold text-slate-900 dark:text-white">{selectedAnnouncement.subject.name}</p>
                  </div>
                </div>
              )}
