...
export default function AdminTimetableOverviewPage() {
  const [rooms, setRooms] = useState<RoomTimetable[]>([])
    ...
    <div className="page-container">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">All rooms schedule</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {rooms.map((cls) => {
            ...
            return (
              <div key={cls.roomId} className="card overflow-hidden">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-slate-800 dark:text-white">{cls.className}</h3>
                  <Link
                    href={`/admin/timetable/builder?roomId=${cls.roomId}`}
